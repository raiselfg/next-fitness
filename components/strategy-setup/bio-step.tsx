...
import { BioStepInput, bioStepSchema } from '@/schema/strategy';
...
export function BioStep({ defaultValues, onNext, onBack }: BioStepProps) {
  const {
    register,
    control,
    handleSubmit,
  ...
  return (
    <form onSubmit={handleSubmit(onNext)} className="flex flex-col grow">
      ...
      <CardFooter className="flex justify-between border-t pt-6 rounded-b-xl px-0 mt-6">
        <Button type="button" variant="ghost" onClick={onBack} disabled={!onBack} className="pl-2">
